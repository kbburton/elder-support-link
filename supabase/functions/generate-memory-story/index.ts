...
${interview.raw_transcript}`;
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o',
        messages: [
          { role: 'system', content: 'You are an expert biographer and storyteller who creates narratives from interview transcripts. You must respond with valid JSON.' },
    ...
    console.log('Parsed story data keys:', Object.keys(storyData));
    console.log('Story title:', storyData.title);
    console.log('Story content length:', storyData.story?.length || 0, 'characters');

    // Validate that we have both title and story content
    if (!storyData.story || storyData.story.trim().length === 0) {
      console.error('ERROR: OpenAI returned empty story content');
      console.error('Full parsed data:', JSON.stringify(storyData, null, 2));
      throw new Error('Story generation failed: No story content returned from AI');
    }
