-- Fix phone uniqueness check to properly handle upserts (which may appear as INSERT)
...
BEGIN
  ...
  IF TG_TABLE_NAME = 'care_groups' THEN
    ...
    -- Check other care_groups (exclude current record)
  ...
  ELSIF TG_TABLE_NAME = 'profiles' THEN
    ...
    -- Check other profiles (ALWAYS exclude current user, regardless of INSERT/UPDATE)
    IF NOT phone_exists THEN
      SELECT EXISTS (
        SELECT 1 FROM profiles 
        WHERE TRIM(phone) = clean_phone 
